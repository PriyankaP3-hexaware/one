  ...
  public static String applyLeave(final int argLeaveEmpId,
    ...
    if (e != null) {
      ...
      if (LeaveType.EL == argLeaveType) {
        ...
        levAvail = e.getEmpMl();
      } else if (LeaveType.OL == argLeaveType) {
        levAvail = e.getEmpOl();
      }
     ...
     // Date cur = new Date();
      ...
      boolean flag = true;
       if(LeaveType.OL == argLeaveType) {
         String[] date ={"2019-05-6","2019-06-7"};
         for(int i = 0; i < date.length; i++){
          Date date1 = sdf.parse(date[i]);
           if(sDate.compareTo(date1) == 0) {
             flag = false;
           }
         }

       }
       if(flag && LeaveType.OL == argLeaveType) {
         res = "you can't apply leave on other";
       } else {
